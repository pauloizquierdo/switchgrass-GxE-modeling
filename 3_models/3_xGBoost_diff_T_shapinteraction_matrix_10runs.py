#%%
# Import libraries for data handling, modeling, and interpretation
import os
import pandas as pd
import numpy as np
...
#%%
# Define traits and environment
traits = range(0, 6)
job_id = int(os.getenv("SLURM_ARRAY_TASK_ID", "0"))
trait_number = traits[job_id]
env = "tx"  # options: "tx", "mi", "diff"

#%%
# Load TPM (transcriptomics) data depending on environment
if env == "tx":
...
pattern = os.path.join(feature_importance, f"{column_name}_GB_featureimportance_{env}_T_shapinter_perc1.csv")
feature_im_cv = pd.read_csv(glob.glob(pattern)[0], index_col=0)

#%%
# Use all features if < 5000 selected
if feature_im_cv.shape[0] < 5000:
    X_train = data_tpm.loc[y_train.index, feature_im_cv.index]
    X_test = data_tpm.loc[y_test.index, feature_im_cv.index]

    # Load pre-trained model
    model = load(glob.glob(os.path.join(models_dir, f"{column_name}_GB_{env}_T_shapinter_perc1.joblib"))[0])
    ...
    # Train new models across seeds for stability
    for i in range(10):
        ...
        shap_df.to_csv(os.path.join(shap_values, f'{column_name}_GB_shap_values_T_{env}_shapinter_allfeatIm_run{i}.csv'))
        ...
        for name, array in {
            ...
            df = pd.DataFrame(array, index=X_train.columns, columns=X_train.columns)
            df.to_csv(os.path.join(shap_values, f'{column_name}_GB_{env}_T_{name}_shapinter_allfeatIm_run{i}.csv'))

    feature_im['Mean'] = feature_im.mean(axis=1)
    feature_im['SD'] = feature_im.std(axis=1)
    feature_im.to_csv(os.path.join(feature_importance, f'{column_name}_GB_featureimportance_T_dataset{env}_allfeatIm_shapinter.csv'))

#%%
# If more than 5000 features, filter to top 5000
else:
    ...
    def fit_and_save_model(best_params, X_train, y_train, X_test, y_test, column_name):
        ...
        dump(model, os.path.join(models_dir, f'{column_name}_GB_T_{env}_shapinter_5k.joblib'))
        ...
        }).to_csv(os.path.join(results_dir, f'{column_name}_GB_metrics_T_{env}_shapinter_perc5k.csv'), index=False)

    fit_and_save_model(best_params, X_train, y_train, X_test, y_test, column_name)

    #%%
    # Load final model and repeat SHAP analysis
    model = load(os.path.join(models_dir, f'{column_name}_GB_T_ap_dataset{env}_shapinter_5k.joblib'))
    ...
    for i in range(10):
        ...
        for name, array in {
            ...
            df = pd.DataFrame(array, index=X_train.columns, columns=X_train.columns)
            df.to_csv(os.path.join(shap_values, f'{column_name}_GB_{env}_{name}_shapinter_allfeatIm_run{i}.csv'))

    feature_im['Mean'] = feature_im.mean(axis=1)
    feature_im['SD'] = feature_im.std(axis=1)
    feature_im.to_csv(os.path.join(feature_importance, f'{column_name}_GB_featureimportance_T_{env}_allfeatIm_shapinter.csv'))
