#%%
# Import required packages for data processing, modeling, and interpretation
import os
import pandas as pd
import numpy as np
import itertools
...
#%%
# Set output directories for saving model artifacts and SHAP-based results
...
#%%
# Generate all combinations of trait and SHAP-based feature selection percentiles
traits = range(0, 6)
feature_sel_per = range(0, 15)
job_id = int(os.getenv("SLURM_ARRAY_TASK_ID", "0"))
combinations = list(itertools.product(traits, feature_sel_per))
trait_number, feature_percen = combinations[job_id]
...
#%%
# Load phenotype data and compute differential if required
env = "tx"
if env == "tx":
...
#%%
# Median imputation for missing phenotype values
...
#%%
# Load previously computed feature importance to select top features
feature_im_cv = pd.read_csv(os.path.join('feature_importance', f'{column_name}_GB_featureimportance_Snps_{env}.csv'), index_col=0)
...
#%%
# Filter SNP features to retain only selected ones from SHAP analysis
x_trn = snps.loc[y_trn_apk_imputed.index, feature_im_cv_NZ.index]
x_tst = snps.loc[y_tst_apk_imputed.index, feature_im_cv_NZ.index]
X_train = x_trn
X_test = x_tst
...
#%%
# Run optimization and extract best hyperparameters
trials = Trials()
...
#%%
# Train the final model, save artifacts, and export evaluation metrics
def fit_and_save_model(best_params, X_train, y_train, X_test, y_test, column_name):
    ...
    dump(model, os.path.join(models_dir, f'{column_name}_GB_Snps_{env}_shapinter_perc{vect_percen[feature_percen]}.joblib'))
    ...
    metrics.to_csv(os.path.join(results_dir, f'{column_name}_GB_metrics_Snps_{env}_shapinter_perc{vect_percen[feature_percen]}.csv'), index=False)

fit_and_save_model(best_params, X_train, y_train, X_test, y_test, column_name)

#%%
# Load trained model and compute feature importances over multiple seeds
model = load(os.path.join(models_dir, f'{column_name}_GB_Snps_{env}_shapinter_perc{vect_percen[feature_percen]}.joblib'))
...
feature_im.to_csv(os.path.join(feature_importance, f'{column_name}_GB_featureimportance_Snps_{env}_shapinter_perc{vect_percen[feature_percen]}.csv'))
...
shap_values_df_model.to_csv(os.path.join(shap_values, f'{column_name}_GB_shap_values_Snps_{env}_shapinter_perc{vect_percen[feature_percen]}.csv'))
